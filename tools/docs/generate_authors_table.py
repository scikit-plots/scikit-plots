...
def get_contributors():
    ...
    comm_team                   = [
        get_profile(login) for login in comm_team
    ]
    emeritus_comm_team          = [
        get_profile(login) for login in emeritus_comm_team
    ]
    contributor_experience_team = [
    ...
    core_devs                   = [
        get_profile(login) for login in core_devs
    ]
    emeritus                    = [
        get_profile(login) for login in emeritus
    ]
    documentation_team          = [
        get_profile(login) for login in documentation_team
    ]
...
if __name__ == "__main__":
    ...
    with open(
        REPO_FOLDER / "docs/source/project/teams" / "communication_team_emeritus.rst", "w+", encoding="utf-8"
    ) as rst_file:
        rst_file.write(generate_list(emeritus_comm_team))

    with open(
        REPO_FOLDER / "docs/source/project/teams" / "contributor_experience_team.rst", "w+", encoding="utf-8"
    ) as rst_file:
    ...
    with open(
        REPO_FOLDER / "docs/source/project/teams" / "maintainers_emeritus.rst", "w+", encoding="utf-8"
    ) as rst_file:
