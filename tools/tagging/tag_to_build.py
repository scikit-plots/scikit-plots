...
def get_last_commit_info():
    ...
    last_commit_id = subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD']).strip().decode()
    last_commit_message = subprocess.check_output(['git', 'log', '-1', '--pretty=%B']).strip().decode()    
...
def check_and_remove_tag(tag):
    try:
        ...
        local_tag_exists = subprocess.run(f"git show-ref --tags | grep -q 'refs/tags/{tag}'", shell=True, check=False)
        ...
        remote_tag_exists = subprocess.run(f"git ls-remote --tags origin {tag}", shell=True, check=False)
