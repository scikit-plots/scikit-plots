...
def save_current_plot(suffix=False, save_plots=False, output='result_images', debug=False):
    """
    Save the current plot if the environment variable to save plots is enabled.

    This function checks if the environment variable 'SAVE_PLOTS' is set. If it is, it saves the 
    current plot to a specified directory, with a filename based on the current script name 
    (or a given suffix). The directory is determined by the 'SAVE_PLOT_DIR' environment variable,
    or defaults to './result_images/'. If the environment variable is not set, the plot is not saved.
    ...
    # Check if SAVE_PLOTS environment variable is set (checking for '1' or 'true') or if save_plots flag is True
    if os.getenv('SAVE_PLOTS', '0').lower() in ['1', 'true'] or save_plots:
