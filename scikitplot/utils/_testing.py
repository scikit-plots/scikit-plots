...
# https://github.com/scikit-learn/scikit-learn/blob/main/sklearn/utils/_tags.py
#
...
def _get_diff_msg(docstrings_grouped):
    ...
    for docstring, group in docstrings_grouped.items():
        ...
        for start, word_group in groupby(diff[3:], key=_diff_key):
            if start is None:
                msg_diff += "\n" + "\n".join(word_group)
            else:
                msg_diff += "\n" + start + " ".join(word[2:] for word in word_group)
