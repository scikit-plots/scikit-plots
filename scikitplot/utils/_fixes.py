...
_IS_32BIT = 8 * struct.calcsize("P") == 32  # noqa: PLR2004
...
def _mode(a, axis=0):
    ...
    if sp_version >= parse_version("1.10.999"):  # noqa: SIM102
...
if sp_base_version >= parse_version("1.12.0"):
    ...
    def _sparse_linalg_cg(A, b, **kwargs):  # noqa: N803
        if "rtol" in kwargs:
...
if sp_base_version >= parse_version("1.11.0"):
    ...
    def _min_or_max_axis(X, axis, min_or_max):
        N = X.shape[axis]  # noqa: N806
        if N == 0:
            raise ValueError("zero-size array to reduction operation")
        M = X.shape[1 - axis]  # noqa: N806
    ...
    def _sparse_min_or_max(X, axis, min_or_max):
        ...
        if axis < 0:
            axis += 2
        if (axis == 0) or (axis == 1):  # noqa: PLR1714
...
def pd_fillna(pd, frame):  # noqa: D103
...
# TODO: remove when SciPy 1.12 is the minimum supported version
def _preserve_dia_indices_dtype(  # noqa: D417
...
def tarfile_extractall(tarfile, path):  # noqa: D103
    try:
...
def _in_unstable_openblas_configuration():
    ...
    import numpy  # noqa: F401, ICN001, PLC0415
