...
def _exec_cmd(  # noqa: PLR0912
    ...
    **kwargs,
    ...
    """
    Get A convenience wrapper of `subprocess.Popen` for running a command from a Python script.

    Parameters
    ----------
    cmd:
        The command to run, as a string or a list of strings.
    throw_on_error:
        If True, raises an Exception if the exit code of the program is nonzero.
    extra_env:
        Extra environment variables to be defined when running the child process.
        If this argument is specified, `kwargs` cannot contain `env`.
    capture_output:
        If True, stdout and stderr will be captured and included in an exception
        message on failure; if False, these streams won't be captured.
    synchronous:
        If True, wait for the command to complete and return a CompletedProcess
    ...
    stream_output:
        If True, stream the command's stdout and stderr to `sys.stdout`
        as a unified stream during execution.
        If False, do not stream the command's stdout and stderr to `sys.stdout`.
    kwargs:
        Keyword arguments (except `text`) passed to `subprocess.Popen`.
...
def cache_return_value_per_process(fn):
    """
    Get A decorator which globally caches the return value of the decorated function.
