...
"""
Machine learning evaluation decile analysis e.g. Gain, Lift and Decile charts, etc.
...
class ModelPlotPy:
    ...
    def prepare_scores_and_ntiles(self):
        ...
        if (len(self.models) == len(self.model_labels)) is False:
            raise ValueError(
                ...
                f"The number of model = {len(self.models)} and model_name = {len(self.model_labels)}."
            )
        ...
        if (
            ...
            raise ValueError(
                ...
                f"{len(self.feature_data)}, label_data = {len(self.label_data)} and description = {len(self.dataset_labels)}."
            )
    ...
    def plotting_scope(  # noqa: D417, PLR0912
        self,
        scope="no_comparison",
        select_model_label=[],
        ...
        if scope == "no_comparison":
            ...
            print(
                f"Target class {select_targetclass[0]}, dataset {select_dataset_label[0]} and model {select_model_label[0]}."
            )
    ...
    **kwargs : dict, optional
    ...
    if scope == "no_comparison":
        ...
        ax.plot(
            ...
            plot_input.pct_ref,
            linestyle="dashed",
            label=f"overall response ({classes[0]})",
            color=colors[0],
        )
    ...
    if highlight_ntile is not None:
        if highlight_ntile not in np.linspace(1, ntiles, num=ntiles).tolist():
            raise TypeError(
                f"Invalid value for highlight_ntile parameter. It must be an int value between 1 and {ntiles}"
            )
...
def plot_cumresponse(
    ...
    **kwargs : dict, optional
    ...
    if scope == "no_comparison":
        ...
        ax.set_title(
            f"Comparing target classes & dataset: {datasets[0]} & model: {models[0]}",
            fontweight="bold",
        )
        ax.legend(loc="upper right", shadow=False, frameon=False)
    ...
    if highlight_ntile is not None:
        if highlight_ntile not in np.linspace(1, ntiles, num=ntiles).tolist():
            raise TypeError(
                ...
                f"It must be an int value between 1 and {ntiles}"
            )
        ...
        if scope == "no_comparison":
            ...
            ax.annotate(
                ...
                va="bottom",
                color="black",
                bbox={"boxstyle": "round, pad = 0.4", "alpha": 1, "fc": colors[0]},
                arrowprops={"arrowstyle": "->", "color": "black"},
            )
            text += (
                f"When we select {description_label} {highlight_ntile} from model {models[0]} "  # noqa: S608
            ...
            for col, i in enumerate(datasets):
                ...
                ax.annotate(
                    ...
                    va="bottom",
                    color="black",
                    bbox={"boxstyle": "round, pad = 0.4", "fc": colors[col], "alpha": 0.8},
                    arrowprops={"arrowstyle": "->", "color": "black"},
                )
                text += (
                    f"When we select {description_label} {highlight_ntile} from model {models[0]} "  # noqa: S608
    ...
    **kwargs : dict, optional
    ...
    if scope == "no_comparison":
        ...
        ax.plot(
            ...
            plot_input.gain_opt,
            linestyle="dashed",
            label=f"optimal gains ({classes[0]})",
            color=colors[0],
            linewidth=1.5,
        )
...
def plot_all(
    ...
    **kwargs : dict, optional
    ...
    if scope == "no_comparison":
        title = f"model: {models[0]} & dataset: {datasets[0]} & target class: {classes[0]}"
        ax1.plot(
        ...
        title = f"scope: comparing datasets & model: {models[0]} & target class: {classes[0]}"
        for col, i in enumerate(datasets):
            ax1.plot(
                plot_input.ntile[plot_input.dataset_label == i],
                plot_input.cumgain[plot_input.dataset_label == i],
        ...
        title = f"scope: comparing models & dataset: {datasets[0]} & target class: {classes[0]}"
        for col, i in enumerate(models):
            ax1.plot(
                plot_input.ntile[plot_input.model_label == i],
                plot_input.cumgain[plot_input.model_label == i],
        ...
        title = f"scope: comparing target classes & dataset: {datasets[0]} & model: {models[0]}"
        for col, i in enumerate(classes):
            ax1.plot(
                plot_input.ntile[plot_input.target_class == i],
                plot_input.cumgain[plot_input.target_class == i],
