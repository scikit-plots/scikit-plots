...
class TestKDE:
    ...
    def test_cached_support(self, rng):
        ...
        npt.assert_array_equal(support, kde.support)
...
class TestHistogram(DistributionFixtures):
    ...
    def test_array_bins(self, x):
        ...
        bin_kws = h.define_bin_params(x)
        npt.assert_array_equal(bin_kws["bins"], bins)
    ...
    def test_bivariate_string_bins(self, x, y):
        ...
        npt.assert_array_equal(e1, np.histogram_bin_edges(x, s1))
        npt.assert_array_equal(e2, np.histogram_bin_edges(y, s1))

        h = Histogram(bins=(s1, s2))
        e1, e2 = h.define_bin_params(x, y)["bins"]
        npt.assert_array_equal(e1, np.histogram_bin_edges(x, s1))
        npt.assert_array_equal(e2, np.histogram_bin_edges(y, s2))
    ...
    def test_bivariate_array_bins(self, x, y):
        ...
        h = Histogram(bins=b1)
        e1, e2 = h.define_bin_params(x, y)["bins"]
        npt.assert_array_equal(e1, b1)
        npt.assert_array_equal(e2, b1)

        h = Histogram(bins=(b1, b2))
        e1, e2 = h.define_bin_params(x, y)["bins"]
        npt.assert_array_equal(e1, b1)
        npt.assert_array_equal(e2, b2)
    ...
    def test_histogram(self, x):
        ...
        npt.assert_array_equal(heights, heights_mpl)
        npt.assert_array_equal(edges, edges_mpl)
    ...
    def test_density_stat(self, x):
        ...
        heights, edges = h(x)
        npt.assert_approx_equal(actual=1, desired=(heights * np.diff(edges)).sum(), significant=7)
    ...
    def test_cumulative_density(self, x):
        ...
        heights, _ = h(x)
        npt.assert_approx_equal(actual=1, desired=heights[-1], significant=7)
    ...
    def test_bivariate_histogram(self, x, y):
        ...
        npt.assert_array_equal(heights, heights_mpl)
        npt.assert_array_equal(edges[0], edges_mpl[0])
        npt.assert_array_equal(edges[1], edges_mpl[1])
...
class TestECDF(DistributionFixtures):
    ...
    def test_univariate_proportion(self, x):
        ...
        ecdf = ECDF()
        stat, vals = ecdf(x)
        npt.assert_array_equal(vals[1:], np.sort(x))
        npt.assert_array_almost_equal(stat[1:], np.linspace(0, 1, len(x) + 1)[1:])
        assert stat[0] == 0
    ...
    def test_univariate_count(self, x):
        ...
        npt.assert_array_equal(vals[1:], np.sort(x))
        npt.assert_array_almost_equal(stat[1:], np.arange(len(x)) + 1)
        assert stat[0] == 0
    ...
    def test_univariate_percent(self, x2):
        ...
        npt.assert_array_equal(vals[1:], np.sort(x2))
        npt.assert_array_almost_equal(stat[1:], (np.arange(len(x2)) + 1) / len(x2) * 100)
        assert stat[0] == 0
    ...
    def test_univariate_proportion_weights(self, x, weights):
        ...
        npt.assert_array_almost_equal(stat[1:], expected_stats)
        assert stat[0] == 0
    ...
    def test_univariate_count_weights(self, x, weights):
        ...
        stat, vals = ecdf(x, weights=weights)
        npt.assert_array_equal(vals[1:], np.sort(x))
        npt.assert_array_almost_equal(stat[1:], weights[x.argsort()].cumsum())
        assert stat[0] == 0
    ...
    def test_against_statsmodels(self, x):
        ...
        ecdf = ECDF()
        stat, vals = ecdf(x)
        npt.assert_array_equal(vals, sm_ecdf.x)
        npt.assert_array_almost_equal(stat, sm_ecdf.y)

        ecdf = ECDF(complementary=True)
        stat, vals = ecdf(x)
        npt.assert_array_equal(vals, sm_ecdf.x)
        npt.assert_array_almost_equal(stat, sm_ecdf.y[::-1])
...
class TestEstimateAggregator:
    ...
    def test_ci_errorbars(self, long_df):
        ...
        npt.assert_array_equal(out_orig, out_test)
...
class TestWeightedAggregator:
    ...
    def test_weighted_mean(self, long_df):
        ...
        npt.assert_array_equal(out["y"], expected)
        npt.assert_array_equal(out["ymin"], np.nan)
        npt.assert_array_equal(out["ymax"], np.nan)
...
class TestLetterValues:
    ...
    def test_levels(self, x):
        ...
        npt.assert_array_equal(res["levels"], expected)

    def test_values(self, x):

        res = LetterValues(k_depth="tukey", outlier_prop=0, trust_alpha=0)(x)
        npt.assert_array_equal(np.percentile(x, res["percs"]), res["values"])
