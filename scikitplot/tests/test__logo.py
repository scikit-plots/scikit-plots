...
Notes
-----
- Private helpers are accessed via ``import scikitplot._logo as _logo_mod``
  (white-box testing) rather than through the public ``sp._logo`` namespace.
...
import scikitplot._logo as _logo_mod
...
class TestPublicConstants:
    ...
    def test_list_size_presets_returns_tuple(self):
        result = sp._logo.list_size_presets()
        assert isinstance(result, tuple)
    ...
    def test_variants_constant_has_four_elements(self):
        assert len(_logo_mod._VARIANTS) == 4

    def test_themes_constant_covers_light_and_dark(self):
        assert set(_logo_mod._THEMES) == {"light", "dark"}

    def test_dots_modes_constant_covers_all_three(self):
        assert set(_logo_mod._DOTS_MODES) == {"fixed", "random", "none"}

    def test_banner_is_non_empty_string(self):
        banner = _logo_mod._SCIKITPLOT_BANNER
    ...
    def test_banner_does_not_start_with_newline(self):
        ...
        """
        assert not _logo_mod._SCIKITPLOT_BANNER.startswith("\n")
    ...
    def test_size_presets_dict_has_required_fields(self):
        ...
        for name, cfg in _logo_mod._SIZE_PRESETS.items():
    ...
    def test_size_presets_dots_values_are_valid(self):
        valid = set(_logo_mod._DOTS_MODES)
        for name, cfg in _logo_mod._SIZE_PRESETS.items():
...
class TestPalette:
    ...
    def test_all_required_keys_present(self, theme, mono):
        P = _logo_mod._palette(theme=theme, mono=mono)
    ...
    def test_background_color_per_theme(self, theme, expected_bg):
        P = _logo_mod._palette(theme=theme, mono=False)
    ...
    def test_mono_background_unchanged(self, theme, expected_bg):
        P = _logo_mod._palette(theme=theme, mono=True)
        assert P["BG"] == expected_bg

    def test_mono_dark_all_fg_colors_identical(self):
        P = _logo_mod._palette(theme="dark", mono=True)
    ...
    def test_mono_light_all_fg_colors_identical(self):
        P = _logo_mod._palette(theme="light", mono=True)
        fg = {k: v for k, v in P.items() if k != "BG"}
    ...
    def test_color_palette_fg_colors_differ(self):
        ...
        P = _logo_mod._palette(theme="light", mono=False)
        fg = {k: v for k, v in P.items() if k != "BG"}
        assert len(set(fg.values())) > 1

    def test_default_is_light_non_mono(self):
        assert _logo_mod._palette() == _logo_mod._palette(theme="light", mono=False)

    def test_light_and_dark_differ(self):
        P_light = _logo_mod._palette(theme="light", mono=False)
        P_dark = _logo_mod._palette(theme="dark", mono=False)
...
class TestInferFormatFromFilename:
    ...
    def test_explicit_ext_overrides_suffix(self):
        fmt, path = _logo_mod._infer_format_from_filename("logo.svg", ext="png", format=None)
        assert fmt == "png"
        assert str(path).endswith(".png")

    def test_explicit_format_used_when_no_ext(self):
        fmt, path = _logo_mod._infer_format_from_filename("logo.svg", ext=None, format="pdf")
        assert fmt == "pdf"
        assert str(path).endswith(".pdf")

    def test_ext_takes_priority_over_format(self):
        fmt, _ = _logo_mod._infer_format_from_filename("logo.svg", ext="png", format="pdf")
        assert fmt == "png"

    def test_infer_from_svg_suffix(self):
        fmt, path = _logo_mod._infer_format_from_filename("logo.svg", ext=None, format=None)
        assert fmt == "svg"
        assert str(path) == "logo.svg"

    def test_infer_from_png_suffix(self):
        fmt, _ = _logo_mod._infer_format_from_filename("logo.png", ext=None, format=None)
        assert fmt == "png"

    def test_infer_from_pdf_suffix(self):
        fmt, _ = _logo_mod._infer_format_from_filename("logo.pdf", ext=None, format=None)
        assert fmt == "pdf"

    def test_default_to_svg_when_no_suffix(self):
        fmt, path = _logo_mod._infer_format_from_filename("logo", ext=None, format=None)
        assert fmt == "svg"
        assert str(path) == "logo.svg"

    def test_uppercase_suffix_normalised_to_lowercase(self):
        fmt, _ = _logo_mod._infer_format_from_filename("logo.SVG", ext=None, format=None)
        assert fmt == "svg"

    def test_ext_with_leading_dot_stripped(self):
        fmt, _ = _logo_mod._infer_format_from_filename("logo.svg", ext=".png", format=None)
        assert fmt == "png"

    def test_format_with_leading_dot_stripped(self):
        fmt, _ = _logo_mod._infer_format_from_filename("logo.svg", ext=None, format=".pdf")
        assert fmt == "pdf"

    def test_accepts_pathlib_path(self):
        fmt, path = _logo_mod._infer_format_from_filename(Path("logo.svg"), ext=None, format=None)
        assert fmt == "svg"
        assert isinstance(path, Path)

    def test_returns_path_object(self):
        _, path = _logo_mod._infer_format_from_filename("logo.svg", ext=None, format=None)
        assert isinstance(path, Path)
...
class TestResolveOutputNames:
    ...
    def test_template_expands_variant_placeholder(self):
        results = _logo_mod._resolve_output_names(
    ...
    def test_single_variant_uses_filename_verbatim(self):
        results = _logo_mod._resolve_output_names("logo.svg", ["primary"], None, None)
        assert len(results) == 1
        assert results[0][1] == "logo.svg"

    def test_format_override_applied(self):
        results = _logo_mod._resolve_output_names("logo.svg", ["primary"], ext="png", format=None)
        assert results[0][2] == "png"
        assert results[0][1].endswith(".png")

    def test_variant_field_in_result(self):
        results = _logo_mod._resolve_output_names("logo.svg", ["knn"], None, None)
...
class TestApplyPreset:
    ...
    def test_none_preset_returns_inputs_unchanged(self):
        result = _logo_mod._apply_preset(
    ...
    def test_explicit_non_default_variant_not_overridden(self):
        ...
        v, _, _, _ = _logo_mod._apply_preset(
    ...
    def test_all_presets_accept_without_error(self):
        for preset in _logo_mod.list_size_presets():
            _logo_mod._apply_preset(
...
class TestCliVariants:
    ...
    def test_all_flag_returns_all_variants(self):
        result = _logo_mod._cli_build_variants(None, all_flag=True)
        assert set(result) == set(_logo_mod._VARIANTS)

    def test_all_flag_length_matches_variants(self):
        result = _logo_mod._cli_build_variants(None, all_flag=True)
        assert len(result) == len(_logo_mod._VARIANTS)

    def test_specific_variants_returned_unchanged(self):
        result = _logo_mod._cli_build_variants(["primary", "knn"], all_flag=False)
        assert result == ["primary", "knn"]

    def test_none_arg_returns_primary_default(self):
        result = _logo_mod._cli_build_variants(None, all_flag=False)
        assert result == ["primary"]

    def test_empty_list_returns_primary_default(self):
        result = _logo_mod._cli_build_variants([], all_flag=False)
        assert result == ["primary"]

    def test_all_flag_overrides_arg_variants(self):
        """--all should take precedence over any --variant arguments."""
        result = _logo_mod._cli_build_variants(["small"], all_flag=True)
        assert set(result) == set(_logo_mod._VARIANTS)

    def test_single_variant_in_list(self):
        result = _logo_mod._cli_build_variants(["metrics"], all_flag=False)
...
class TestDraw:
    ...
    def test_draw_all_variants_produces_patches(self, variant):
        fig, ax = plt.subplots()
        sp._logo.draw(ax, variant=variant)
    ...
    def test_draw_mono_true_produces_patches(self):
        fig, ax = plt.subplots()
        sp._logo.draw(ax, mono=True)
        assert len(ax.patches) > 0

    def test_draw_mono_false_produces_patches(self):
        fig, ax = plt.subplots()
        sp._logo.draw(ax, mono=False)
        assert len(ax.patches) > 0
    ...
    def test_draw_random_dots_same_seed_same_count(self):
        fig1, ax1 = plt.subplots()
        sp._logo.draw(ax1, variant="primary", dots="random", seed=42)
        n1 = len(ax1.patches)
        plt.close(fig1)

        fig2, ax2 = plt.subplots()
        sp._logo.draw(ax2, variant="primary", dots="random", seed=42)
        n2 = len(ax2.patches)
        plt.close(fig2)
    ...
    def test_draw_facecolor_matches_light_palette_bg(self):
        P = _logo_mod._palette(theme="light")
        fig, ax = plt.subplots()
        sp._logo.draw(ax, theme="light")
    ...
    def test_draw_facecolor_matches_dark_palette_bg(self):
        P = _logo_mod._palette(theme="dark")
        fig, ax = plt.subplots()
        sp._logo.draw(ax, theme="dark")
        actual = mpl.colors.to_hex(ax.get_facecolor())
        expected = mpl.colors.to_hex(mpl.colors.to_rgba(P["BG"]))
        assert actual == expected
    ...
    def test_draw_always_adds_outer_circle(self):
        ...
        for variant in _logo_mod._VARIANTS:
            fig, ax = plt.subplots()
            sp._logo.draw(ax, variant=variant)
    ...
    def test_draw_primary_adds_wedge_patches(self):
        fig, ax = plt.subplots()
        sp._logo.draw(ax, variant="primary")
    ...
    def test_draw_small_adds_wedge_patches(self):
        fig, ax = plt.subplots()
        sp._logo.draw(ax, variant="small")
        wedges = [p for p in ax.patches if isinstance(p, Wedge)]
        assert len(wedges) == 3
    ...
    def test_draw_knn_adds_extra_lines_beyond_trend(self):
        ...
        fig1, ax1 = plt.subplots()
        sp._logo.draw(ax1, variant="primary")
        n_primary_lines = len(ax1.lines)
        plt.close(fig1)

        fig2, ax2 = plt.subplots()
        sp._logo.draw(ax2, variant="knn")
    ...
    def test_draw_small_has_fewer_artists_than_primary(self):
        ...
        fig1, ax1 = plt.subplots()
        sp._logo.draw(ax1, variant="small")
...
class TestCreate:
    ...
    def test_all_presets_no_error(self, preset):
        fig, ax = sp._logo.create(preset=preset)
        assert fig is not None

    def test_favicon_preset_produces_smaller_figure_than_default(self):
        fig_favicon, _ = sp._logo.create(preset="favicon")
        fig_default, _ = sp._logo.create()
        assert fig_favicon.get_figwidth() < fig_default.get_figwidth()

    def test_invalid_preset_raises_value_error(self):
        with pytest.raises(ValueError, match="preset must be one of"):
            sp._logo.create(preset="invalid")  # type: ignore[arg-type]

    def test_invalid_variant_raises_value_error(self):
        with pytest.raises(ValueError, match="variant must be one of"):
            sp._logo.create(variant="bad")  # type: ignore[arg-type]
...
class TestSave:
    ...
    def test_save_returns_list(self, tmp_path):
        result = sp._logo.save(tmp_path / "logo.svg")
    ...
    def test_save_ext_override_changes_format(self, tmp_path):
        out = tmp_path / "logo.svg"
        paths = sp._logo.save(out, ext="png")
        assert paths[0].endswith(".png")
        assert Path(paths[0]).exists()

    def test_save_format_param_changes_format(self, tmp_path):
        out = tmp_path / "logo"
        paths = sp._logo.save(out, format="svg")
    ...
    def test_save_single_variant_param(self, tmp_path):
        out = tmp_path / "logo.svg"
        paths = sp._logo.save(out, variant="small")
        assert len(paths) == 1

    def test_save_variants_string_treated_as_single(self, tmp_path):
        out = tmp_path / "logo.svg"
        paths = sp._logo.save(out, variants="small")
        assert len(paths) == 1
    ...
    def test_save_template_expansion(self, tmp_path):
        ...
        paths = sp._logo.save(tmpl, variants=["primary", "small"])
        assert len(paths) == 2
    ...
    def test_save_closes_created_figures(self, tmp_path):
        ...
        sp._logo.save(tmp_path / "logo.svg", variants=["primary", "small"])
...
class TestWordmark:
    ...
    def test_create_docs_hero_preset_bumps_text_size(self):
        ...
        fig, _ = sp._logo.wordmark.create(preset="docs-hero", text_size=30)
        assert fig is not None

    def test_create_size_controls_figwidth(self):
        fig, _ = sp._logo.wordmark.create(size=4.0)
        assert fig.get_figwidth() == pytest.approx(4.0)

    # ── show() ────────────────────────────────────────────────────────────────

    def test_show_returns_figure(self):
        fig = sp._logo.wordmark.show()
        assert isinstance(fig, mpl.figure.Figure)

    def test_show_passes_kwargs_to_create(self):
        fig = sp._logo.wordmark.show(theme="dark")
        assert fig is not None
    ...
    def test_save_svg_creates_file(self, tmp_path):
        ...
        result = sp._logo.wordmark.save(out)
        assert out.exists()
    ...
    def test_save_ext_override(self, tmp_path):
        out = tmp_path / "lockup.svg"
        result = sp._logo.wordmark.save(out, ext="png")
        assert result.endswith(".png")
        assert Path(result).exists()

    def test_save_format_override(self, tmp_path):
        out = tmp_path / "lockup"
        result = sp._logo.wordmark.save(out, format="svg")
        assert result.endswith(".svg")

    def test_save_docs_hero_preset_creates_file(self, tmp_path):
        out = tmp_path / "lockup.svg"
        result = sp._logo.wordmark.save(out, preset="docs-hero")
        assert Path(result).exists()
    ...
    def test_save_docs_hero_preset_wider_than_default(self, tmp_path):
        ...
        sp._logo.wordmark.save(out_default)
        sp._logo.wordmark.save(out_hero, preset="docs-hero")
    ...
    def test_save_closes_figure(self, tmp_path):
        ...
        before = len(plt.get_fignums())
        sp._logo.wordmark.save(tmp_path / "lockup.svg")
        after = len(plt.get_fignums())
        assert after == before

    def test_save_dark_theme(self, tmp_path):
        out = tmp_path / "lockup-dark.svg"
        result = sp._logo.wordmark.save(out, theme="dark")
        assert Path(result).exists()
...
class TestDeterminism:
    ...
    def test_all_variants_deterministic(self, variant):
        fig1, ax1 = sp._logo.create(variant=variant)
        fig2, ax2 = sp._logo.create(variant=variant)
        fig1.canvas.draw()
        fig2.canvas.draw()
        assert _ax_signature(ax1) == _ax_signature(ax2)

    def test_same_seed_random_dots_artists_identical(self):
        fig1, ax1 = sp._logo.create(variant="primary", dots="random", seed=42)
        fig2, ax2 = sp._logo.create(variant="primary", dots="random", seed=42)
        fig1.canvas.draw()
        fig2.canvas.draw()
        assert _ax_signature(ax1) == _ax_signature(ax2)
    ...
    def test_theme_deterministic(self, theme):
        fig1, ax1 = sp._logo.create(theme=theme)
        fig2, ax2 = sp._logo.create(theme=theme)
        assert _ax_signature(ax1) == _ax_signature(ax2)

    def test_mono_deterministic(self):
        fig1, ax1 = sp._logo.create(mono=True)
        fig2, ax2 = sp._logo.create(mono=True)
        assert _ax_signature(ax1) == _ax_signature(ax2)
    ...
    def test_primary_save_svg_produces_same_file_twice(self, tmp_path):
        ...
        a = tmp_path / "a.svg"
        b = tmp_path / "b.svg"
        sp._logo.save(a, variant="primary", dots="fixed")
        sp._logo.save(b, variant="primary", dots="fixed")
...
class TestCLI:
    ...
    def test_main_creates_output_dir_if_missing(self, tmp_path):
        ...
        result = sp._logo.main(["--out", str(subdir)])
        assert result == 0
        assert subdir.exists()

    def test_main_dots_none(self, tmp_path):
        result = sp._logo.main(["--out", str(tmp_path), "--dots", "none"])
        assert result == 0

    def test_main_dots_random(self, tmp_path):
        result = sp._logo.main(["--out", str(tmp_path), "--dots", "random"])
        assert result == 0

    def test_main_wordmark_all_presets(self, tmp_path):
        for preset in sp._logo.list_size_presets():
            out = tmp_path / preset
            out.mkdir()
            result = sp._logo.main([
...
class TestEdgeCases:
    ...
    def test_save_single_and_multiple_variants_dont_conflict(self, tmp_path):
        ...
        sp._logo.save(out_single, variant="primary")
        size_after_single = out_single.stat().st_size

        sp._logo.save(tmp_path / "logos.svg", variants=["primary", "small"])
    ...
    def test_wordmark_icon_ratio_respected(self):
        ...
        """
        ratio = 0.35
        _, (ax_icon, _) = sp._logo.wordmark.create(icon_ratio=ratio)
    ...
    def test_draw_primary_fixed_dots_all_inside_circle(self):
        ...
        inner_r = 0.93
        for x, y, _r, _key in _logo_mod._FIXED_DOTS:
            dist = (x**2 + y**2) ** 0.5
            assert dist < inner_r, (
...
class TestBannerStructure:
    ...
    def test_banner_is_str_type(self):
        assert isinstance(_logo_mod._SCIKITPLOT_BANNER, str)

    def test_banner_has_at_least_four_lines(self):
        lines = _logo_mod._SCIKITPLOT_BANNER.splitlines()
    ...
    def test_banner_does_not_start_with_whitespace_only_line(self):
        ...
        first = _logo_mod._SCIKITPLOT_BANNER.splitlines()[0]
    ...
    def test_banner_line_count_is_stable(self):
        ...
        """
        n1 = len(_logo_mod._SCIKITPLOT_BANNER.splitlines())
        n2 = len(_logo_mod._SCIKITPLOT_BANNER.splitlines())
        assert n1 == n2
...
class TestLerpColorEdgeCases:
    ...
    def test_t_greater_than_one_extrapolates_beyond_c2(self):
        ...
        """
        result = _logo_mod._lerp_color("black", "white", 1.5)
        assert isinstance(result, np.ndarray)
        assert result.shape == (3,)
    ...
    def test_result_is_always_ndarray_shape_3(self):
        ...
        for t in [-1.0, -0.5, 0.0, 0.25, 0.5, 0.75, 1.0, 1.5]:
            r = _logo_mod._lerp_color("red", "blue", t)
    ...
    def test_named_colors_accepted(self):
        ...
        result = _logo_mod._lerp_color("navy", "steelblue", 0.5)
        assert result.shape == (3,)
        assert np.all(result >= 0) and np.all(result <= 1)
    ...
    def test_symmetry_at_half(self):
        ...
        """
        r1 = _logo_mod._lerp_color("red", "blue", 0.5)
        r2 = _logo_mod._lerp_color("blue", "red", 0.5)
...
class TestPaletteExtended:
    ...
    def test_palette_is_deterministic(self):
        ...
        p1 = _logo_mod._palette(theme="light", mono=False)
        p2 = _logo_mod._palette(theme="light", mono=False)
        assert p1 == p2
...
class TestDrawExtended:
    ...
    def test_primary_has_more_bars_than_small(self):
        ...
        fig2, ax2 = plt.subplots()
        sp._logo.draw(ax2, variant="small", dots="none")
    ...
    def test_small_has_no_orange_dots(self):
        ...
        fig, ax = plt.subplots()
        sp._logo.draw(ax, variant="small", dots="none")
    ...
    def test_knn_has_exactly_two_graph_edges(self):
        ...
        sp._logo.draw(ax_p, variant="primary")
        n_primary_lines = len(ax_p.lines)
        plt.close(fig_p)

        fig_k, ax_k = plt.subplots()
        sp._logo.draw(ax_k, variant="knn")
    ...
    def test_random_dots_different_seeds_may_differ(self):
        ...
        """
        fig1, ax1 = plt.subplots()
        sp._logo.draw(ax1, variant="primary", dots="random", seed=0)
        sig1 = _ax_signature(ax1)
        plt.close(fig1)

        fig2, ax2 = plt.subplots()
        sp._logo.draw(ax2, variant="primary", dots="random", seed=99)
...
class TestCreateExtended:
    ...
    def test_create_returns_new_figure_each_call(self):
        ...
        fig1, _ = sp._logo.create()
        fig2, _ = sp._logo.create()
    ...
    def test_create_avatar_preset_size(self):
        ...
        fig, _ = sp._logo.create(preset="avatar")
    ...
    def test_create_docs_hero_preset_size(self):
        ...
        fig, _ = sp._logo.create(preset="docs-hero")
    ...
    def test_create_figure_is_square(self):
        ...
        for size in (2.0, 4.0, 6.0):
            fig, _ = sp._logo.create(size=size)
    ...
    def test_create_dots_none_allowed(self):
        ...
        fig, ax = sp._logo.create(dots="none")
        assert fig is not None

    def test_create_mono_does_not_error(self):
        fig, ax = sp._logo.create(mono=True, theme="dark")
        assert fig is not None
...
class TestSaveExtended:
    ...
    def test_save_returns_list_of_strings_not_paths(self, tmp_path):
        ...
        paths = sp._logo.save(tmp_path / "logo.svg")
    ...
    def test_save_all_four_files_exist_on_disk(self, tmp_path):
        ...
        paths = sp._logo.save(tmpl, variants=list(sp._logo.list_variants()))
        for p in paths:
...
class TestWordmarkExtended:
    ...
    def test_create_default_icon_ratio_is_0_28(self):
        ...
        _, (ax_icon, _) = sp._logo.wordmark.create()
    ...
    def test_create_height_is_fraction_of_width(self):
        ...
        """
        size = 6.0
        fig, _ = sp._logo.wordmark.create(size=size)
    ...
    def test_create_text_axes_is_axis_off(self):
        ...
        _, (_, ax_text) = sp._logo.wordmark.create()
    ...
    def test_create_docs_hero_raises_text_size_to_44(self):
        ...
        """
        fig, _ = sp._logo.wordmark.create(preset="docs-hero", text_size=10)
        assert isinstance(fig, mpl.figure.Figure)
    ...
    def test_save_mono_creates_file(self, tmp_path):
        ...
        result = sp._logo.wordmark.save(out, mono=True)
        assert Path(result).exists()
