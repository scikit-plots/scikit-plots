...
_ALL_FINDINGS = frozenset(
    {
        "CYTHON-CON-001", "CYTHON-CACHE-001", "CYTHON-GC-001", "CYTHON-LOAD-002",
        "CYTHON-SEC-001", "CYTHON-API-001", "CYTHON-CACHE-002", "CYTHON-PIN-001",
        "CYTHON-WASM-001", "CYTHON-LOAD-001", "CYTHON-CACHE-003", "CYTHON-TPL-001",
        "CYTHON-PKG-001", "CYTHON-CON-002", "CYTHON-RES-001", "CYTHON-CACHE-004",
        "CYTHON-COMP-001", "CYTHON-TYP-001", "CYTHON-SEC-002", "CYTHON-SCH-001",
        "CYTHON-API-002", "CYTHON-API-003", "CYTHON-BATCH-001", "CYTHON-DOC-001",
        "CYTHON-PORT-001", "CYTHON-TPL-002", "CYTHON-OBS-001", "CYTHON-ABI-001",
        "CYTHON-TEST-001", "CYTHON-PERF-001",
    }
)
...
class TestWellFormed:
    ...
    def test_required_sections(self) -> None:
        ...
        for section in ("document map", "change workflow", "prevention rules",
                        "review log", "adding an adr"):
...
class TestCompleteReviewLog:
    ...
    def test_no_unknown_finding_codes(self) -> None:
        ...
        assert not unknown, f"MAINTAINING.md references unknown findings: {sorted(unknown)}"
...
class TestNoDanglingReferences:
    def test_referenced_docs_exist(self) -> None:
        ...
        missing = [
            ...
            if "/" not in name and not (pkg / name).exists()
        ]
