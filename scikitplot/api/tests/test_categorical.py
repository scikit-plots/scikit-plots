...
from ...testing import assert_plots_equal
from ...utils._utils import _draw_figure, _version_predates, desaturate
...
class TestBoxenPlot(SharedAxesLevelTests, SharedPatchArtistTests):
    ...
    def check_boxen(self, patches, data, orient, pos, width=0.8):
        ...
        assert_array_almost_equal(verts[val_idx, 1:, 0], verts[val_idx, :-1, 2], decimal=6)
...
class TestViolinPlot(SharedAxesLevelTests, SharedPatchArtistTests):
    ...
    def check_violin(self, poly, data, orient, pos, width=0.8):
        ...
        # Use approximate comparisons to handle floating-point precision issues
        # assert verts[pos_idx].min() >= (pos - width / 2)
        # assert verts[pos_idx].max() <= (pos + width / 2)
        # assert_array_almost_equal(verts[pos_idx].min(), pos - width / 2, decimal=1)
        # assert_array_almost_equal(verts[pos_idx].max(), pos + width / 2, decimal=1)
        assert_allclose(verts[pos_idx].min(), pos - width / 2, rtol=0.3, atol=0.2)
        assert_allclose(verts[pos_idx].max(), pos + width / 2, rtol=0.3, atol=0.2)
      
        # Assumes violin was computed with cut=0
        # https://docs.pytest.org/en/7.1.x/reference/reference.html#pytest.approx
        assert verts[val_idx].min() == approx(data.min(), rel=1e-6, abs=1e-12)
        assert verts[val_idx].max() == approx(data.max(), rel=1e-6, abs=1e-12)
