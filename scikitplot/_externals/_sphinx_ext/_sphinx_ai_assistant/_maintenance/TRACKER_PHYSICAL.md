...
- `_sphinx_llm` must not import `_sphinx_ai_assistant`;
- the assistant must not import private `*_sphinx_llm.upstream.*` implementation;
- browser bundles must not contain configured production secret values.

## Extraction ratchet

When canonical Markdown/llms generation moves to `_sphinx_llm`, record the
removed assistant responsibilities and tests in the same checkpoint. Do not
leave two canonical implementations active indefinitely.
