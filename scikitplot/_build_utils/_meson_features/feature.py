...
# ----------------------------------------------------------------------
# Makes ALL annotations in this file strings at runtime (PEP 563 / 749).
# Required because:
#
#   1. Python 3.8 / 3.9 do not support the built-in generic aliases
#      (list[str], dict[str,str], set[...]) or X|Y union syntax in
#      annotations — those are 3.9+ / 3.10+ respectively.  The typing
#      module forms (List, Dict, Set, Union, Optional) are used here for
#      the same reason, and remain legal through Python 3.13+.  They are
#      soft-deprecated (PEP 585) but not removed.
#
#   2. Python 3.14+ will make lazy annotation evaluation the default
#      (PEP 749).  Having this import in place now means behaviour is
#      identical on 3.8–3.13 and 3.14+.
#
#   3. The meson decorator system (@typed_kwargs, @typed_pos_args) does
#      NOT use Python annotations for runtime type checking.  It uses the
#      explicit KwargInfo runtime objects.  Making annotations lazy is
#      therefore safe for all meson decorator call sites.
# ============================================================================
...
from .. import ModuleObject # type: ignore[]

if TYPE_CHECKING:
    import sys as _sys
    from typing import TypedDict
    # NotRequired was added to the standard typing module in Python 3.11
    # (PEP 655).  On 3.8–3.10 we fall back to typing_extensions which
    # provides an identical backport.  This block is only evaluated by
    # static type checkers (mypy, pyright) — never at runtime — so the
    # conditional import does not affect any Python version at run time.
...
class FeatureObject(ModuleObject):
    ...
    def __init__(self, state: 'ModuleState',
        ...
        def init_attrs(state: 'ModuleState',
            ...
            self.disable: str = kwargs['disable'] # type: ignore[]
            if not self.detect:
                if self.group:
    ...
    def get_implicit(self, _caller: Optional[Set[FeatureObject]] = None
                     ) -> Set[FeatureObject]:
        # Recursion guard: features can imply each other, so we track
        # which features are already on the call stack.  _caller starts
        # as None (not an empty set) so callers never have to pass it.
        # With `from __future__ import annotations` the forward reference
        # quotes around 'FeatureObject' are no longer needed.
    ...
    def sorted_multi(features: Iterable[Union['FeatureObject', Iterable['FeatureObject']]],
        ...
        def sort_cb(k: Union[FeatureObject, Iterable[FeatureObject]]) -> int:
            ...
            # multi features this why this function isn't considered
            # accurate.
