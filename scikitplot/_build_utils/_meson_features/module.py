...
#
# That fork predates several breaking API changes that were later merged into
# upstream (stock) mesonbuild.  When this package uses stock pip-installed
# meson (>= ~1.5.0) instead of NumPy's vendored binary, two internal compiler
# API call sites in this file and in utils.py need adjustment.
#
# Stock meson resolves  import('features')  via:
#   importlib.import_module('mesonbuild.modules.features')
# There is no plugin path or MESON_MODULE_PATH env-var.  The only correct
# deployment is to copy this package into <mesonbuild.modules location>/features/
# which is what install_meson_features.py does at configure time.
#
# Breaking API changes fixed in this file
# ----------------------------------------
# 1. CLikeCompiler.has_multi_arguments(args)                 [module.py L403]
#    Old numpy/meson fork signature : has_multi_arguments(args, env)
#    Stock meson >= ~1.5 signature  : has_multi_arguments(args)
#    Reason for upstream change     : The compiler object now holds a
#    reference to its own Environment internally (self.environment is set
#    during compiler detection in environment.py).  Passing env explicitly
#    became redundant and was removed from the public signature.
#    Fix: drop the  state.environment  positional argument.
#
# 2. Compiler.cached_compile(code, *, extra_args)            [utils.py L29]
#    Old numpy/meson fork signature : cached_compile(code, coredata, extra_args=...)
#    Stock meson >= ~1.5 signature  : cached_compile(code, *, extra_args=..., mode=..., temp_dir=...)
#    Reason for upstream change     : coredata is accessed via  self.environment.coredata
#    inside the compiler object.  Accepting it as an external positional arg
#    was an early design that was superseded when compilers gained a stable
#    self.environment reference.  Removing it also made the API keyword-only
#    after the first positional (code), preventing future accidental
#    positional mis-ordering.
#    Fix: drop the  state.environment.coredata  positional argument in utils.py.
#
# API calls confirmed stable in stock meson 1.10.1 (no changes needed)
# ----------------------------------------------------------------------
#   state._interpreter.func_static_lib(node, args, kwargs) — unchanged
#   state.environment.coredata                              — unchanged (instance attr)
#   state.environment.build_dir / state.subdir             — unchanged
#   CompileResult.cached / .returncode / .stderr           — unchanged
#   MachineChoice.HOST                                      — unchanged
#   coredata.compilers[for_machine]                        — unchanged
#   All relative imports (mesonlib, interpreterbase, etc.) — unchanged
#
# How to verify after a meson upgrade
# ------------------------------------
# Run the following and compare against the call sites above:
#   python -c "
#   import inspect
#   from mesonbuild.compilers.mixins.clike import CLikeCompiler
#   from mesonbuild.compilers.compilers import Compiler
#   print(inspect.signature(CLikeCompiler.has_multi_arguments))
#   print(inspect.signature(Compiler.cached_compile))
#   "
# If either signature changes again, update the corresponding call site
# and add a note to this header.
# ============================================================================
#
# ============================================================================
...
# The meson decorator system (@typed_kwargs, @typed_pos_args) uses
# KwargInfo runtime objects for type enforcement — not Python annotations
# — so making annotations lazy has no effect on meson's type checking.
# ============================================================================
...
from ... import mlog, build # type: ignore[]
from ...compilers import Compiler # type: ignore[]
from ...mesonlib import File, MesonException # type: ignore[]
...
if TYPE_CHECKING:
    ...
    from ...interpreterbase import TYPE_var, TYPE_kwargs # type: ignore[]
...
class TargetsObject(ModuleObject):
    ...
    def static_lib_method(self, state: 'ModuleState', args: Tuple[str],
        ...
        for features in FeatureObject.sorted_multi(self._targets.keys()):
            link_whole += self._targets[cast(tcast, features)] # type: ignore[]
        if not link_whole:
            return []
...
class Module(NewExtensionModule):
    ...
    def test_any(self, state: 'ModuleState', features: Set[FeatureObject],
                 ...
                 force_args: Optional[List[str]],
                 cached: bool,
                 # dummy no need for recursive guard
                 _caller: Optional[Set[FeatureObject]] = None,
                 ) -> 'TestResultKwArgs':
    ...
    def test(self, state: 'ModuleState', features: Set[FeatureObject],
        ...
        # implied features already tested also we use this set to generate
        # unique target name that can be used for multiple targets
        # build.
        ...
        # since we allows features to imply each wither
                ...
                for conflict in values:
                    ...
                    for acc in accumulate_values:
                        ...
                        if not (conflict.match and conflict.match.match(acc)):
                            new_acc.append(acc)
                            continue
                        # no filter so we totally escape it
                        ...
                        # no filter match so we totally escape it
                        if not filter_val:
                            continue
        ...
        if args:
            ...
            #
            # The numpy/meson fork of CLikeCompiler.has_multi_arguments
            # accepted an explicit Environment object as its second positional
            # argument so it could look up coredata for cached results.
            #
            # Stock meson refactored the compiler class hierarchy so that every
            # Compiler instance holds a stable self.environment reference set
            # during compiler detection (environment.py:detect_compiler_for).
            # Accepting env externally became redundant.
            #
            # Stock meson >= ~1.5.0 signature:
            #   has_multi_arguments(self, args: List[str]) -> Tuple[bool, bool]
            #
            # Return value (bool, bool) = (is_supported, was_cached) is
            # unchanged.  Dropping the second positional arg is the only fix.
            # ----------------------------------------------------------------
