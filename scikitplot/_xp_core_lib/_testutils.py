...
import scikitplot
IS_EDITABLE = 'editable' in scikitplot.__path__[0]
...
def check_free_memory(free_mb):
    ...
    try:
        mem_free = _parse_size(os.environ['SKPLT_AVAILABLE_MEM'])
        msg = '{} MB memory required, but environment SKPLT_AVAILABLE_MEM={}'.format(
            free_mb, os.environ['SKPLT_AVAILABLE_MEM'])
    except KeyError:
        mem_free = _get_mem_available()
        if mem_free is None:
            pytest.skip("Could not determine available memory; set SKPLT_AVAILABLE_MEM "
