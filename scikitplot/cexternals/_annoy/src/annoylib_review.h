...
/* =========================================================================================
 * ENHANCED ANNOYLIB - COMPREHENSIVE TYPE SUPPORT
 * =========================================================================================
 *
 * Features:
 ...
 * - self.y_map dict-like default null if user provide when add index y=float or double match same index
 * - self.y list like y get filled None or null unless add build if not fill
 *
 * Constructor Signature:
 *
 *   AnnoyIndex(
 *       f=None,                 // Dimension (inferred from first vector if 0/None) get_f() set_f(f=Null) try self.f does not affect
 *       metric=None,            // "angular", "euclidean", "manhattan", "dot", "hamming" get_metric() -> "angular" set_metric(Null) self.metric default "angular"
 *       *,                      // Keyword-only args below
 *       n_trees=-1,             // Default trees for queries if this provided call into method like build(n_trees=Null) use fron self.n_trees
 *       n_neighbors=5,          // Default neighbors for queries self.on_disk_path
 *       on_disk_path=None,      // Default filename on-disk build if provided -> on_disk_build(filename=Null) use self.on_disk_path
 *       prefault=None,          // Prefault pages when loading Null or false build(prefault=Null) use fron self.prefault
 *       seed=None,              // Random seed 0 use in set_seed(Null) self.seed
 *       verbose=None,           // Verbosity level false or 0 use in verbose(Null) self.verbosity
 *       schema_version=None,    // Serialization schema version null or 0 self.schema_version for serialize /deserialize can be empty str o int
 *       index_dtype='int32',    // Index type: 'int32', 'int64', 'float', 'double'  self.index_dtype
 *       hamming_dtype='uint32', // 'uint32', 'uint64', 'uint8', 'bool'  self.hamming_dtype if metric hamming enforce hamming wrapper we need bitwise proccess
 *       dtype='float'           // Data type: 'float', 'double', 'float16', 'float128', 'bool' self.dtype
 *   )
 *
 *   HammingWrapper(
 *       f=None,
 ...
 *       schema_version=None,
 *       index_dtype='int32',
 *       hamming_dtype='uint32', // 'uint32', 'uint64', 'uint8', 'bool'
 *       dtype='float'           // External API type
 *   )
 *
 * ========================================================================================= */
...
// Float16 support (IEEE 754 half precision)
...
//   // C++11 compile-time type safety checks
 ...
 * Valid types: int32_t, int64_t, float, double
 */
template<typename S>
 ...
 * Valid types: bool, uint8_t, float16_t, float, double, float128_t, uint32_t, uint64_t
 */
template<typename T>
...
// Valid Hamming storage types: uint32_t, uint64_t, uint8_t, bool
template<typename T>
struct is_valid_hamming_type {
  static constexpr bool value =
    std::is_same<T, uint32_t>::value ||
    std::is_same<T, uint64_t>::value ||
    std::is_same<T, uint8_t>::value ||
...
// Default values for lazy construction and Python API compatibility
...
inline T safe_numeric_cast(S value) {
  // Integer to integer: direct cast with bounds checking
...
/* ================================================================
 * Distance metric helper functions (SIMD-optimized)
 * ================================================================ */
...
inline Node* get_node_ptr(const void* _nodes, const size_t _s, const S i) {
  // return (Node*)((uint8_t *)_nodes + (_s * i));  // Replace C-style casts:
  return reinterpret_cast<Node*>(
    reinterpret_cast<const uint8_t*>(_nodes) + (_s * i)
  );
}
...
inline T dot(const T* x, const T* y, int f) {
  static_assert(std::is_arithmetic<T>::value,
                "dot() requires arithmetic type");
  if (f <= 0) {
    return T(0);
  }
  T result = DefaultValue<T>::get();
  for (int i = 0; i < f; i++) {
    // result += x[i] * y[i];
    result += (*x) * (*y);
    x++;
...
inline T manhattan_distance(const T* x, const T* y, int f) {
  ...
  for (int i = 0; i < f; i++) {
    // result += fabs(x[i] - y[i]);
    result += (x[i] > y[i]) ? (x[i] - y[i]) : (y[i] - x[i]);
  }
  return result;
}

// Specialization for bool (Hamming distance)
template<>
inline uint32_t manhattan_distance<bool>(const bool* x, const bool* y, int f) {
  uint32_t result = 0;
  for (int i = 0; i < f; i++) {
    result += (x[i] != y[i]) ? 1 : 0;
  }
...
inline float manhattan_distance<float>(const float* x, const float* y, int f) {
  ...
  // Don't forget the remaining values.
  for (; i > 0; i--) {
    // result += fabs(*x - *y);  // fabsf
    float diff = (*x) - (*y);
    result += (diff >= 0.0f) ? diff : -diff;
    x++;
    y++;
  }
  return result;
}
#endif // ANNOYLIB_USE_AVX
...
#endif // ANNOYLIB_USE_AVX512

/* ================================================================
 * Two-means clustering helper
 * ================================================================ */
...
inline void two_means(const vector<Node*>& nodes, int f, Random& random,
  ...
  */
  ...
  static constexpr int iteration_steps = 200;
  ...
  size_t i = random.index(static_cast<uint32_t>(count));
  size_t j = random.index(static_cast<uint32_t>(count - 1));
  ...
  for (int l = 0; l < iteration_steps; l++) {
    size_t k = random.index(static_cast<uint32_t>(count));
...
/* ================================================================
 * Distance Metric Base Class and Implementations
 * ================================================================ */
...
struct Base {
  ...
  static inline void zero_value(Node* dest) noexcept {
    // Initialize any fields that require sane defaults within this node.
    static_assert(std::is_trivially_copyable<Node>::value,
                  "Node must be trivial or zero-initializable POD");
    std::memset(dest, 0, sizeof(Node));
  }
  /* ------------------------------------------------------------
   * Safe node copy
   * ------------------------------------------------------------ */
  ...
  static inline void copy_node(Node* dest, const Node* source, const int f) {
    ...
    std::memcpy(dest->v, source->v, static_cast<size_t>(f) * sizeof(T));
    // dest->v must be allocated for f * sizeof(T)
    // memcpy(dest->v, source->v, f * sizeof(T));
  }
  template<typename T, typename Node>
  static inline T get_norm(Node* node, int f) {
    static_assert(std::is_floating_point<T>::value,
                  "Norm only valid for floating-point metrics");
    // T acc = dot(node->v, node->v, f);
    T acc = T(0);
    for (int i = 0; i < f; ++i)
      acc += node->v[i] * node->v[i];
    return std::sqrt(acc);
  }
  template<typename T, typename Node>
  static inline void normalize(Node* node, int f) {
    T norm = get_norm<T>(node, f);
    if (norm == T(0)) return;
    if (norm > T(0)) {
      for (int z = 0; z < f; z++)
        node->v[z] /= norm;
        //node->v[z] = safe_divide(node->v[z], norm, T(0));
    }
  }
  template<typename T, typename Node>
  static inline void update_mean(Node* mean, Node* new_node, T norm, int c, int f) {
    ...
    T c_plus_1 = static_cast<T>(c + 1);
    for (int z = 0; z < f; z++) {
      mean->v[z] = (mean->v[z] * c + safe_divide(new_node->v[z], norm, T(0))) / c_plus_1;
    }
  }
};
...
struct Angular : Base {
  ...
  static constexpr bool coordinate_wise = false;
  ...
  static inline T distance(const Node<S, T>* x, const Node<S, T>* y, int f) {
    ...
    T ppqq = pp * qq;
    if (ppqq > T(0)) {
      T cos_sim = safe_divide(pq, std::sqrt(ppqq), T(0));
      return T(2.0) - T(2.0) * cos_sim;
    } else {
      return T(2.0); // cos is 0
    }
  }
  ...
  template<typename S, typename T, typename Random>
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s,
                                  Random& random, Node<S, T>* n) {
    // Replace raw casts:
    // Node<S, T>* p = (Node<S, T>*)alloca(s);
    // Node<S, T>* q = (Node<S, T>*)alloca(s);
    auto* p = reinterpret_cast<Node<S, T>*>(alloca(s));
    auto* q = reinterpret_cast<Node<S, T>*>(alloca(s));
  ...
  static inline T normalized_distance(T distance) {
    ...
    return std::sqrt(std::max(distance, T(0)));
  }
...
struct DotProduct : Angular {
  ...
  static inline void normalize(Node* node, int f) {
    ...
    T norm = sqrt(dot(node->v, node->v, f) + (node->dot_factor * node->dot_factor));
    if (norm > 0) {
      for (int z = 0; z < f; z++)
        node->v[z] /= norm;
...
struct Hamming : Base {
  ...
  static inline T distance(const Node<S, T>* x, const Node<S, T>* y, int f) {
    static_assert(sizeof(T) == 4 || sizeof(T) == 8,
                  "Hamming distance requires 32-bit or 64-bit integer type");
    size_t dist = 0;
    for (int i = 0; i < f; i++) {
      // dist += annoylib_popcount(x->v[i] ^ y->v[i]);
      dist += static_cast<T>(annoylib_popcount(x->v[i] ^ y->v[i]));
    }
    return dist;
  }
  ...
  static inline bool margin(const Node<S, T>* n, const T* y, int f) {
    static_assert(sizeof(T) == 4 || sizeof(T) == 8,
                  "Hamming metric requires 32-bit or 64-bit integer type");

    static const size_t n_bits = sizeof(T) * 8;
    // n->v[0] stores a bit index, not a chunk index.
...
/*
 * IMPORTANT INVARIANT (Minkowski / Euclidean / Manhattan):
 * n->a += -n->v[z] * (p->v[z] + q->v[z]) / 2;
 * The split hyperplane normal is L2-normalized before computing the offset.
 * This means:
 *
 *   - The plane equation is scale-normalized
 *   - Side tests and margin ordering are preserved
 *   - Exact geometric distances to the plane are NOT preserved
 *
 * This is intentional:
 * Annoy uses hyperplanes only for ranking and traversal ordering,
 * not for exact geometric distance computations.
 *
 * Do NOT remove normalization unless you also update all margin and
 * pq_distance semantics.
 */
...
struct Euclidean : Minkowski {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n) {
    ...
    n->a = 0.0;
    for (int z = 0; z < f; z++)
      // ⚠️ Acceptable only if invariant is:
      // Plane equation is normalized for distance ranking, not exact geometry
      n->a += -n->v[z] * (p->v[z] + q->v[z]) / 2;
  }
  template<typename T>
  static inline T normalized_distance(T distance) {
...
/* ================================================================
 * AnnoyIndexInterface - Pure virtual base class
 *
 * Template Parameters:
 *   S: Index type (int32_t | int64_t | float | double)
 *   T: Data type (bool | uint8_t | float16_t | float | double | float128_t | uint32_t | uint64_t)
 *   R: Random seed type (uint32_t | uint64_t)
 * ================================================================ */
...
class AnnoyIndexInterface {
  ...
  static_assert(is_valid_data_type<T>::value,
                "T must be bool, uint8_t, float16_t, float, double, float128_t, uint32_t, or uint64_t");
  static_assert(is_valid_random_seed_type<R>::value,
                "R must be uint32_t or uint64_t");
#endif
 public:
  ...
  virtual int get_f() const = 0;
  virtual int set_f(f=NULL)
  virtual char* get_metric() = "angular"
  virtual bool add_item(S item=0, const T* w=NULL, char** error=NULL) = 0;
  virtual bool build(int q=10, int n_threads=-1, char** error=NULL) = 0;
  virtual bool unbuild(char** error=NULL) = 0;
  virtual bool save(const char* filename="annoy.annoy", bool prefault=false, char** error=NULL) = 0;
  virtual void unload() = 0;
  virtual bool load(const char* filename="annoy.annoy", bool prefault=false, char** error=NULL) = 0;
  virtual T get_distance(S i=0, S j=0) const = 0;
  virtual void get_nns_by_item(S item=0, size_t n=5, int search_k=-1, vector<S>* result=NULL, vector<T>* distances=NULL) const = 0;
  virtual void get_nns_by_vector(const T* w=NULL, size_t n=5, int search_k=-1, vector<S>* result=NULL, vector<T>* distances=NULL) const = 0;
  virtual S get_n_items() const = 0;
  virtual S get_n_trees() const = 0;  // int or S
  virtual void verbose(bool v = false) = 0;
  virtual void get_item(S item=0, T* v=NULL) const = 0;
  virtual void set_seed(R q=0) = 0;
  virtual bool on_disk_build(const char* filename="annoy.annoy", char** error=NULL) = 0;
...
/* ================================================================
 * Forward declaration of build policies
 * ================================================================ */
...
/* ================================================================
 * AnnoyIndex - Main index implementation with lazy construction
 *
 * Template Parameters:
 *   S: Index type (int32_t | int64_t | float | double)
 *   T: Data type (bool | uint8_t | float16_t | float | double | float128_t | uint32_t | uint64_t)
 *   Distance: Metric (Angular | Euclidean | Manhattan | DotProduct | Hamming)
 *   Random: RNG (Kiss32Random | Kiss64Random)
...
public:
  // Default constructor (lazy initialization - dimension inferred from first add_item)
  AnnoyIndex()
      : _f(0)
      , _f_inferred(false)
      , _s(0)
  ...
  explicit AnnoyIndex(int f = 0)
    : _f(f < 0 ? 0 : f),           // Normalize negative to lazy (0)
      _f_inferred(false),
      _s(f > 0 ? _calculate_node_size(f) : 0),
      _n_items(0),
      _n_nodes(0),
      _nodes_size(0),
      _roots(),
      ...
      _fd(0),
      _nodes(NULL),
      _K(f > 0 ? (S)((sizeof(T) * f + sizeof(S) * 2) / sizeof(T)) : 0),
#if __cplusplus >= 201103L
      _build_failed(false),
#endif
      _seed(Random::default_seed) {
  ...
  {
    if (f < 0) {
      throw std::invalid_argument("Dimension f must be non-negative (use 0 for lazy inference)");
    }
  ...
  // Set dimension explicitly (for lazy mode where f was 0)
  bool set_dimension(int f, char** error = NULL) {
    if (_loaded) {
      set_error_from_string(error, "Cannot set dimension on a loaded index");
      return false;
    }

    if (_n_items > 0) {
      set_error_from_string(error, "Cannot change dimension after adding items");
      return false;
    }

    if (f <= 0) {
      set_error_from_string(error, "Dimension must be positive");
      return false;
    }

    _f = f;
    _s = _calculate_node_size(f);
    _K = (S)((sizeof(T) * f + sizeof(S) * 2) / sizeof(T));

    return true;
  }

  // Get current dimension
  int get_f() const override {
    return _f;
  }

  bool add_item(S item, const T* w, char** error=NULL) override {
    // Lazy dimension inference on first add_item
  ...
  bool add_item_impl(S item, const W& w, char** error=NULL) {
    ...
    if (item < 0) {
      set_error_from_string(error, "Item index must be non-negative");
      return false;
    }

    // Lazy dimension inference from first add_item
    if (_f == 0 && !_f_inferred) {
      // User must provide dimension explicitly
      set_error_from_string(error, "Dimension not specified. Use AnnoyIndex(f) or call set_f(f) before add_item");
      return false;
    }

    if (_f == 0) {
      set_error_from_string(error, "Dimension must be set before adding items recall or use set_dimension()");
      return false;
    }
  ...
  bool set_f(int f, char** error = NULL) {
    if (_n_items > 0) {
      set_error_from_string(error, "Cannot change dimension after items have been added");
      return false;
    }

    if (_loaded) {
      set_error_from_string(error, "Cannot change dimension of a loaded index");
      return false;
    }
  ...
  bool on_disk_build(const char* file, char** error=NULL) {
    if (filename == NULL) {
      set_error_from_string(error, "Filename cannot be NULL");
      return false;
    }

    if (_f == 0) {
      set_error_from_string(error, "Dimension must be set before on_disk_build");
      return false;
    }
    _on_disk = true;
#ifndef _MSC_VER
  ...
  bool build(int q, int n_threads=-1, char** error=NULL) {
    ...
    if (_f == 0) {
      set_error_from_string(error, "Dimension must be set before building");
      return false;
    }

    if (_n_items == 0) {
      set_error_from_string(error, "Cannot build index with zero items");
      return false;
    }
    ...
    if (_build_failed) {
#endif
      set_error_from_string(error, "Build failed");
    return true;
  }
  ...
  bool save(const char* filename="annoy.annoy", bool prefault=false, char** error=NULL) {
    if (!_built) {
  ...
  bool _infer_dimension(const T* w, int& inferred_f) {
    // Simple heuristic: assume non-zero values indicate actual dimension
    // This is a placeholder - real implementation would need user guidance
    inferred_f = 0;
    return false; // Disabled by default - user must provide dimension
  }
  ...
  bool load(const char* filename, bool prefault=false, char** error=NULL) {
    ...
    if (prefault) {
#ifdef __linux__
      if (madvise(_nodes, size, MADV_WILLNEED) == -1) {
      }
#endif
    }
...
protected:
  bool _reallocate_nodes(S n) {
    ...
    if (_on_disk) {
      ...
      if (new_bytes > old_bytes) {
        // memset((char*)_nodes + old_bytes, 0, new_bytes - old_bytes);  // Replace C-style casts:
        memset(reinterpret_cast<char*>(_nodes) + old_bytes, 0, new_bytes - old_bytes);
      }
    }
  ...
  inline Node* _get(const S i) const {
    ...
    return reinterpret_cast<Node*>(static_cast<char*>(_nodes) + static_cast<size_t>(_s) * static_cast<size_t>(i));
  }

  // Rename _split_imbalance → _split_balance_ratio
  double _split_imbalance(const vector<S>& left_indices,
    ...
    const double ls = static_cast<double>(left_indices.size());
    const double rs = static_cast<double>(right_indices.size());
    const double total = ls + rs;

    // Deterministic guard: no items means maximal imbalance
    // float f = ls / (ls + rs + 1e-12);  // Avoid 0/0
    if (total == 0.0) {
      return 1.0;
    }
    const double frac = ls / total;
  ...
  S _make_tree(const vector<S>& indices, bool is_root, Random& _random, ThreadedBuildPolicy& threaded_build_policy) {
    ...
    auto* m = reinterpret_cast<Node*>(alloca(_s));
  ...
  void _get_all_nns(const T* v, size_t n, int search_k, vector<S>* result, vector<T>* distances) const {
    ...
    auto* v_node = reinterpret_cast<Node*>(alloca(_s));
...
// ⚠️ move wrapper below the interface.
// ---------------------------------------------------------------------
// HammingWrapper - Generic Template Adapter with lazy construction
//
...
//
//   S: Index type (int32_t | int64_t | float | double)
//   T: External data type (float | double) - user-facing API
//   InternalT: Internal storage type (uint8_t | uint32_t | uint64_t)
//   Random: RNG (Kiss32Random | Kiss64Random)
//   ThreadedBuildPolicy: Build threading policy
//
// Canonical semantics:
//
...
//   - Internal dimension = ceil(f_external / 64)
...
template<typename S = int32_t,
         typename T = float,
         typename InternalT = uint32_t,
         typename Random = Kiss64Random,
         typename ThreadedBuildPolicy = AnnoyIndexThreadedBuildPolicy>
class HammingWrapper final : public AnnoyIndexInterface<S, T,
#if __cplusplus >= 201103L
  typename std::remove_const<decltype(Random::default_seed)>::type
#else
  typename Random::seed_type
...
private:
  ...
  S _f_external;  // number of bits
  S _f_internal;  // number of uint64_t words
  bool _f_inferred;
...
public:
  ...
  bool set_f(int f, char** error = NULL) {
    ...
    _f_internal = (_f_external + BITS_PER_WORD - 1) / BITS_PER_WORD;
    _f_inferred = true;
  ...
  // -------------------- AnnoyIndexInterface ------------------------
  bool add_item(S item, const T* embedding, char** error = NULL) override {
    try {
      if (embedding == NULL) {
        if (error != NULL) {
      ...
      vector<InternalT> packed(static_cast<size_t>(_f_internal));
      _pack(embedding, packed.data());
      return _index.add_item(indice, packed.data(), error);
  ...
  bool build(int n_trees = -1, int n_threads = -1, char** error = NULL) override {
  ...
  // -------------------- Serialization ------------------------------
  std::vector<uint8_t> serialize(char** error) const override {
    HammingHeader hdr{};
    ...
    vector<uint8_t> out(sizeof(hdr));
    std::memcpy(out.data(), &hdr, sizeof(hdr));

    vector<uint8_t> payload = _index.serialize(error);
  ...
  bool deserialize(vector<uint8_t>* bytes, bool prefault = false, char** error = NULL) override {
  ...
  // -------------------- Disk I/O -----------------------------------
  bool load(const char* filename="annoy.annoy",
            bool prefault=false,
            char** error = NULL) override {
    return _index.load(filename, prefault, error);
  }

  bool save(const char* filename="annoy.annoy",
            bool prefault=false,
            char** error = NULL) override {
    return _index.save(filename, prefault, error);
  }

  bool on_disk_build(const char* filename="annoy.annoy",
                     char** error = NULL) override {
  ...
  void set_seed(Random seed=0) override {
    _index.set_seed(seed);
  }

  void verbose(bool v=0) override {
    _index.verbose(v);
  }
};
...
class AnnoyIndexMultiThreadedBuildPolicy {
private:
  // ⚠️ shared_timed_mutex is deprecated in C++20
