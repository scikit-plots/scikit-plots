...
import ast, collections, json, pathlib, re, sys
...
def imports(path: pathlib.Path, mine: str, known: "set[str]"):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            elif node.level == 0 and node.module and node.module.startswith("scikitplot."):
...
def main() -> int:
    ...
    for mod in mods:
        for path in (root / mod).rglob("*"):
            if not path.is_file() or "__pycache__" in path.parts or "_backup" in path.parts:
                continue
    ...
    graph: dict = {}
    print("%-12s %-12s %-14s %5s %5s" % ("FROM", "TO", "KIND", "code", "test"))
    for a in sorted(edges):
        for b in sorted(edges[a]):
            for kind in ("module-scope", "deferred", "cython-extern"):
                ...
                print("%-12s %-12s %-14s %5d %5d" % (a, b, kind, code, len(items) - code))
                graph.setdefault(a, {}).setdefault(b, {})[kind] = {
                    "code": code, "test": len(items) - code,
    ...
    def visit(node: str) -> bool:
        if node in stack:
            print(f"CYCLE detected at {node!r}", file=sys.stderr)
            return True
        if node in seen:
            return False
    ...
    if any(visit(n) for n in list(graph)):
        return 1
    print("no cycles: the review order in DEPENDENCY_MAP.md is a topological sort")
    return 0
