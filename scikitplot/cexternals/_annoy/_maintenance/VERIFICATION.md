# Verification — `scikitplot.cexternals._annoy`
...
$ python -m pytest scikitplot/cexternals -q -p no:cacheprovider
```

Expected from the gate:

```text
scikitplot.cexternals._annoy: tracker matches the tree (29 source / 14 test files, ...)
```
...
```console
$ python scikitplot/cexternals/_maintenance/check_trackers.py --update
```
