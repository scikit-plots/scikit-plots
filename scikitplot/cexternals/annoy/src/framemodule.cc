...
static PyObject* get_nns_to_python(
  ...
  // Construct a tuple (result, distances)
...
// get_nns_by_item → DataFrame-like NNSFrame if include_distances!=0, else list[int]
static PyObject* py_an_get_nns_by_item(py_annoy* self, PyObject* args, PyObject* kwargs) {
  int32_t item_idx;
  int32_t n;
  int32_t search_k = -1;
  int32_t include_distances = 1;

  static const char* kwlist[] = {"i", "n", "search_k", "include_distances", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "ii|ii",
                                   (char**)kwlist, &item_idx, &n, &search_k, &include_distances))
    return NULL;
  ...
  self->ptr->get_nns_by_item(item_idx, (size_t)n, search_k,
                             &result, include_distances ? &distances : NULL);
  Py_END_ALLOW_THREADS;

  if (!include_distances) {
    Py_ssize_t sz = (Py_ssize_t)result.size();
    PyObject* list = PyList_New(sz);
    if (!list) return NULL;
    for (Py_ssize_t i = 0; i < sz; ++i)
      PyList_SET_ITEM(list, i, PyLong_FromLong(result[(size_t)i]));
    return list;
  }

  AnnoyQueryResult rr(std::move(result),
                      include_distances ? std::move(distances)
                                        : std::vector<float>());
  return py_nnsresult_new_from_cpp(rr);
}

// get_nns_by_vector → same semantics as get_nns_by_item
static PyObject* py_an_get_nns_by_vector(py_annoy* self, PyObject* args, PyObject* kwargs) {
  PyObject* v;
  int32_t n;
  int32_t search_k = -1;
  int32_t include_distances = 1;

  static const char* kwlist[] = {"vector", "n", "search_k", "include_distances", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "Oi|ii",
                                   (char**)kwlist, &v, &n, &search_k, &include_distances))
    return NULL;

  if (!self->ptr) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Annoy index is not initialized. Call add_item() or load() first.");
    return NULL;
  }
  if (self->f == 0) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Cannot query nearest neighbors before dimension is known. Call add_item() first.");
    return NULL;
  }

  std::vector<float> w((size_t)self->f);
  if (!convert_list_to_vector(self, v, self->f, &w))
    return NULL;
  ...
  Py_BEGIN_ALLOW_THREADS;
  self->ptr->get_nns_by_vector(&w[0], (size_t)n, search_k,
                               &result, include_distances ? &distances : NULL);
  Py_END_ALLOW_THREADS;
