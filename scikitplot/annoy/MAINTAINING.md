...
| Metric aliases (R21 follow-up) | `annoy/_annoy/annoylib.pyx.in` | P2 | 11 documented metric aliases are accepted but the two 'Invalid metric' errors were inconsistent/under-informative (one partial, one none). Both now list canonical names + note documented aliases + point to the docstring. Regression: `tests/test_metric_aliases.py`. Ring green. |
| float16 runtime dispatch (Run 25) | `cexternals/_annoy/src/annoylib.h` | P1 (crash) | float16 chose F16C at COMPILE time; with cpu-dispatch=max this baked F16C/AVX512 into variants that SIGILL on hosts not honoring them (crashed the whole _annoy suite in the uploaded build). Now compiles scalar + F16C (via target attr) and selects at RUNTIME via `__builtin_cpu_supports`. SIGILL resolved (434 passed). AVX512 dispatch left as-is. Regression: `tests/test_float16_portable_fallback.cpp` + `tests/test_float_capability_report.cpp`. |
| supported_dtypes() API + multiprecision tier (CY-016) | `cexternals/_annoy/src/annoy_type_support.h` + `annoy/_annoy/annoylib.pyx.in` | P2 | Runtime float-type capability registry surfaced as `supported_dtypes()`; full ladder float16..float512 with tier/size/precision. float80 native (not-yet-dtype); float256/512 gated behind boost multiprecision (honest 'unavailable', never a silent long-double alias). Regression: `_annoy/tests/test_supported_dtypes.py` (13) + `cexternals/_annoy/tests/test_type_support.cpp`. |
| float80 usable dtype (CY-016) | `cexternals/_annoy/src/annoylib.h` + `annoy/_annoy/annoylib.{pxd,pyx}.in` | P2 | Wired float80_t (long double, 80-bit x87) as a usable dtype via the data-driven data_types matrix; added long double to is_valid_data_type + HammingWrapper whitelist; TypeName guarded against MSVC float128==long double collision. supported_dtypes() reports it usable. Regression: `_annoy/tests/test_supported_dtypes.py` (17) + `cexternals/_annoy/tests/test_type_support.cpp`. |
| CY-002 unconsumed .pxi | `annoy/_annoy/annoylib.pxi` + `meson.build` | P2 | annoylib.pxi was fs.copyfile'd but never `include`d; its DEF constants were unused and stale (DEFAULT_SCHEMA_VERSION=1 vs live schema_version=0). Removed file + meson copyfile; build unaffected (proves unconsumed). Regression: `_annoy/tests/test_no_orphan_pxi.py`. |
| native-error ownership sweep (R14/R19 follow-up) | `annoy/_annoy/annoylib.pyx.in` | P2 | Migrated all GIL-held raw `char* error`+manual free() sites to ScopedError RAII: save, load, _ensure_index on_disk_build, unbuild, get_state (serialize), set_state (deserialize). The 2 nogil sites (add_item, build) keep the idiomatic raw pointer (freed after the block) with a clarifying note. Regression: `_annoy/tests/test_save_load_scopederror.py` (5). |
| CY-020 warning volume | `cexternals/_annoy/src/annoylib.h` | P2 | ~35 -Wno-* suppressions on the shared generated TU hid actionable warnings in our C++. Strict-compiled the hand-written headers alone; fixed 2 ftruncate `warn_unused_result` best-effort sites (`(void)` doesn't silence it on GCC). Intentional _random thread-local shadow + benign conversions documented. Guard: `cexternals/_annoy/tests/test_warning_budget.cpp` (CI: -Werror=unused-result/return-type/uninitialized). |
| __contains__/__getitem__ consistency (R15 follow-up) | `annoy/_annoy/annoylib.pyx.in` | P3 | __getitem__ supported negative indexing but __contains__ rejected negatives, contradicting its own docstring. __contains__ now returns `-len <= item < len` (True iff self[item] accessible). Regression: `_annoy/tests/test_contains_getitem_consistency.py` (32). |
| CY-003 installed .pxd ABI policy | `annoy/_annoy/annoylib.pxd.in` | P2 | The installed generated .pxd (1502 lines, 218 decls incl. 160+ AnnoyIndex typedefs) exposed the internal template surface with no stability statement. Decision: PRIVATE implementation detail. Added an INTERNAL/NO-ABI-stability policy block to the pxd header (carried into the generated/installed .pxd), pointing users to the Python `Index` API; kept the `devel` install tag. Regression: `_annoy/tests/test_pxd_abi_policy.py`. |
| CY-001 duplicate template removed | `annoy/_annoy/annoylib_pyx.in` | P1 | A stale non-built twin of annoylib.pyx.in (still had raw free(error), missing supported_dtypes/float80/ScopedError) could hide fixes from review. Removed it; build unaffected (proves it controlled nothing packaged). Guard: `_annoy/tests/test_single_canonical_template.py`. |
| CY-009 concurrency policy | `annoy/_annoy/annoylib.pyx.in` | P1 | build()/query claimed 'thread-safe' but nogil native calls aren't mutually excluded (query-while-build etc. race). Adopted Policy A: corrected docstrings + added explicit Concurrency policy (safe: independent instances + concurrent reads on a built index; unsafe: any mutator overlapping same-instance op). Guard: `_annoy/tests/test_concurrency_policy.py`. |
...
Two substantial extensions are intentionally deferred and recorded in `_maintenance/DEFERRED_FUTURE_WORK.md`:
- **(A)** multiprecision backend (boost/MPFR) to make float256/512 real dtypes;
- **(B)** widened public I/O bridge so >float64 precision reaches add_item/get_item.
