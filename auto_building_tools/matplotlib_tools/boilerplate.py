    ...
    **kwargs
                    ...
                    if param.name == "data"
                        ...
                        if param.kind in [Parameter.POSITIONAL_OR_KEYWORD, Parameter.KEYWORD_ONLY]
                            ...
                            if param.kind is Parameter.POSITIONAL_ONLY
                                ...
                                else "**{0}" if param.kind is Parameter.VAR_KEYWORD else None
                            )
                        )
                    )
                )
    ...
    for reserved in ("gca", "gci", "gcf", "__ret"):
        if reserved in params:
            raise ValueError(f"Method {called_fullname} has kwarg named {reserved}")
...
def boilerplate_gen():
    ...
    cmappable = {
        "contour": "if __ret._A is not None:  # type: ignore[attr-defined]\n" "        sci(__ret)",
        "contourf": "if __ret._A is not None:  # type: ignore[attr-defined]\n" "        sci(__ret)",
        "hexbin": "sci(__ret)",
        "scatter": "sci(__ret)",
        "pcolor": "sci(__ret)",
        ...
        "hist2d": "sci(__ret[-1])",
        "imshow": "sci(__ret)",
        "spy": "if isinstance(__ret, cm.ScalarMappable):\n" "        sci(__ret)",
        "quiver": "sci(__ret)",
    ...
    for spec in _axes_commands:
        ...
        template = AXES_CMAPPABLE_METHOD_TEMPLATE if name in cmappable else AXES_METHOD_TEMPLATE
        yield generate_function(
            name, f"Axes.{called_name}", template, sci_command=cmappable.get(name)
        )
...
def build_pyplot(pyplot_path):
    ...
    subprocess.run([sys.executable, "-m", "black", "--line-length=88", pyplot_path], check=True)
...
def update_sig_from_node(node, sig):
    ...
    if node.returns is not None:
        return inspect.Signature(
            params.values(), return_annotation=direct_repr(ast.unparse(node.returns))
        )
    else:
