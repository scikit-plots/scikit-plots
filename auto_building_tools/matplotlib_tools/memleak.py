...
def run_memleak_test(bench, iterations, report):
    ...
    for i in range(endi):
        ...
        print(
            f"{i: 4d}: pymalloc {malloc: 10d}, rss {rss: 10d}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("backend", type=str, nargs=1, help="backend to test")
    parser.add_argument("iterations", type=int, nargs=1, help="number of iterations")
    parser.add_argument("report", type=str, nargs=1, help="filename to save report")
    parser.add_argument(
        "--empty",
        action="store_true",
