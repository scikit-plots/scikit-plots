...
class Bench(Benchmark):
    params = [[20, 100, 500, 1000], ["contig", "nocont"], ["numpy", "scipy"]]
...
class Lstsq(Benchmark):
    ...
    def setup(self, dtype, size, lapack_driver):
        ...
        n = math.ceil(2.0 / 3.0 * size)
        k = math.ceil(1.0 / 2.0 * size)
        m = size

        if dtype is np.complex128:
            A = (10 * rng.random((m, k)) - 5) + 1j * (10 * rng.random((m, k)) - 5)
            temp = (10 * rng.random((k, n)) - 5) + 1j * (10 * rng.random((k, n)) - 5)
            b = (10 * rng.random((m, 1)) - 5) + 1j * (10 * rng.random((m, 1)) - 5)
        else:
    ...
    def time_lstsq(self, dtype, size, lapack_driver):
        if lapack_driver == "numpy":
            np.linalg.lstsq(self.A, self.b, rcond=np.finfo(self.A.dtype).eps * 100)
        else:
            sl.lstsq(
                self.A,
                self.b,
...
class SpecialMatrices(Benchmark):
    ...
    def setup(self, size):
        ...
        self.big_blocks = [np.ones([size // 2, size // 2]), np.ones([size // 2, size // 2])]
