...
from .common import Benchmark, run_monitored, safe_import, set_mem_rlimit
...
class GridDataPeakMem(Benchmark):
    ...
    def setup(self):
        ...
        sparse_matrix = csr_matrix(
            (random_values, (random_rows, random_cols)), shape=shape, dtype=np.float32
        )
...
class RBFInterpolator(Benchmark):
    ...
    def time_rbf_interpolator(self, neighbors, n_samples, kernel):
        interp = interpolate.RBFInterpolator(
            self.y, self.d, neighbors=neighbors, epsilon=5.0, kernel=kernel
        )
        interp(self.x)
...
class CloughTocherInterpolatorValues(interpolate.CloughTocher2DInterpolator):
    ...
    def __init__(self, points, xi, tol=1e-6, maxiter=400, **kwargs):
        interpolate.CloughTocher2DInterpolator.__init__(
            self, points, None, tol=tol, maxiter=maxiter
        )
        self.xi = None
