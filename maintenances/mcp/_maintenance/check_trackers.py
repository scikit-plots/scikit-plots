...
"""
Re-derive the physical tracker from the tree and fail on drift or boundary leak.
...
MAX_MODULE_LOC = 1200
MAX_ROOT_LOC_SHARE = 0.95  # MCP is intentionally flat; almost all code is root-level
MIN_TEST_SOURCE_RATIO = 0.50


def corpus_root() -> pathlib.Path:
    """Return the ``scikitplot/mcp`` directory containing this script."""
...
def tripwires(  # ruff: ignore[undocumented-param]
    actual: dict,
    tracker_exceptions: set[str] = frozenset(),
) -> list[str]:
    """
    ...
    Parameters
    ----------
    tracker_exceptions : set of str
        Subpackages with a **recorded** justification in ``TRACKER.json``.
        An exception must be written down to be honoured -- silently passing
        would defeat the gate, and an undocumented exception is indistinguishable
        from an oversight.
    """
    crossed: list[str] = []
    ...
    for name, counts in sorted(actual["subpackages"].items()):
        if name in ("(root)", "_maintenance", "tests"):
            continue
        if name in tracker_exceptions:
            continue  # recorded in TRACKER.json -> physical.known_exceptions
...
#: Modules permitted to import the MCP SDK or its wire-model library.  Exactly
#: one, by contract: a second importer makes the ``[mcp]`` extra non-optional
#: for every consumer of ``scikitplot.mcp``.
SDK_IMPORTERS = {"_server.py"}

#: Module-scope import of any of these collapses the injection seam.
#: ``scikitplot.corpus`` and ``annoy`` reach MCP through the ``DocsRetriever``
#: Protocol at call time, never by import.
FORBIDDEN_ROOTS = ("corpus", "annoy")

#: A real module-scope import, as opposed to the same words in prose.
_IMPORT_RE = re.compile(r"^(?:import|from)\s+([.\w]+)")

#: Tooling, not runtime: excluded from the boundary rules it enforces.
_EXCLUDED_DIRS = ("_maintenance", "tests")


def _module_scope_imports(path: pathlib.Path) -> list[tuple[int, str]]:
    """
    Yield ``(line_number, imported_module)`` for unindented imports only.

    Notes
    -----
    Indentation is the signal: a deferred (call-time) import is indented, and
    that is precisely the pattern this module relies on to keep optional
    dependencies optional.  Comments and docstring prose are skipped, so the
    words "scikitplot.corpus" in a docstring are not a violation -- only an
    actual statement is.
    """
    found = []
...
def check_boundary(root: pathlib.Path) -> list[str]:
    """
    Verify the two import rules that define this module's boundary.

    Notes
    -----
    Both rules are true today and were, until this gate, protected by nothing.
    That is the condition under which a boundary decays silently: the offending
    import works fine on the machine that adds it.
    """
    problems: list[str] = []
    for path in sorted(root.rglob("*.py")):
        rel = str(path.relative_to(root))
        if (
            ...
            or rel.split("/", maxsplit=1)[0] in _EXCLUDED_DIRS
        ):
            continue

        for number, module in _module_scope_imports(path):
            head = module.lstrip(".").split(".")[0]

            if head in ("mcp", "pydantic") and rel not in SDK_IMPORTERS:
                problems.append(
                    f"{rel}:{number} imports {module!r} at module scope; only "
                    f"{sorted(SDK_IMPORTERS)} may do so, or the [mcp] extra "
                    "stops being optional for every consumer"
                )

            tail = module.lstrip(".").split(".")
            if head in FORBIDDEN_ROOTS or (
                len(tail) > 1 and tail[1] in FORBIDDEN_ROOTS
            ):
                problems.append(
                    f"{rel}:{number} imports {module!r} at module scope; "
                    "retrieval backends are injected through DocsRetriever and "
                    "imported at call time, never at module scope"
                )
    return problems
...
def main(argv: list[str] | None = None) -> int:
    ...
    boundary = check_boundary(root)
    exceptions = set(tracker["physical"].get("known_exceptions", {}))
    crossed = tripwires(actual, exceptions)
    ...
    if not (drift or crossed or boundary or logical):
        totals = actual["totals"]
        print(  # ruff: ignore[print]
