...
$ python -m pytest scikitplot/random -q -p no:cacheprovider
```

Expected from the gate:

```text
scikitplot.random: tracker matches the tree (5 source / 4 test files, ...)
```
...
```console
$ python scikitplot/random/_maintenance/check_trackers.py --update
```
